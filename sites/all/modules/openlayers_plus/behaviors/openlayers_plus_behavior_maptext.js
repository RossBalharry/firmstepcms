      ...
      if (data && data.map.behaviors.openlayers_plus_behavior_maptext) {
        ...
        for (region in regions) {
          ...
          if (regions[region]['showpanel'] == 1) {
            ...
            positions = regions[region]['panelfieldset']['position'].split(" ");
            i = 0;
            for (pos in positions) {
              panel.div.classList.add(positions[i]);
              i++;
            }
          }
          /**
...
OpenLayers.Control.mtext = OpenLayers.Class(OpenLayers.Control, {
  ...
  draw: function() {
    ...
    if (this.groupDiv == null) {
      ...
      i = 0;
      for (pos in positions) {
       this.div.classList.add(positions[i]);
       i++;
      }
...
OpenLayers.Control.MaptextPanel = OpenLayers.Class(OpenLayers.Control.Panel, {
    
    somefunction: function(options) {

    },
    CLASS_NAME: "OpenLayers.Control.MaptextPanel"
});
